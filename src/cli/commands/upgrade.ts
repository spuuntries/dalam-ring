...
export default defineCommand({
  meta: {
    name: "upgrade",
    description: "Upgrade an invite to a full membership",
  },
  args: {
    ring: {
      type: "string",
      description: "URL of a ring member to bootstrap from",
      required: true,
    },
  ...
  run: async ({ args }) => {
    ...
    const op = createKeyClaimOp(args.url, publicKey, seenIds, privateKey);
